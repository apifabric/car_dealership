...
resourcename="cardealership"  # lower case, only
resourcegroup="cardealership_rg"
...
# git clone https://github.com/apilogicserver/cardealership.git
# cd cardealership
...
az group create --name cardealership_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group cardealership_rg --sku S1 --is-linux
az container create --resource-group cardealership_rg --name cardealership --image apilogicserver/cardealership:latest --dns-name-label cardealership --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//cardealership.westus.azurecontainer.io:5656'
